...
export default async () => {
  // --- SEGURO PARA EVITAR ERROR EADDRINUSE ---
  if (global.webServerRunning) return; 
...
logger.post('/delete-bot', async (req, res) => {
  ...
  if (!phone) return res.status(400).json({ message: 'Número de teléfono no proporcionado' });
  const pho = normalizePhoneForPairing(phone);
  const botSessionPath = path.join(__dirname, 'Sessions', 'Subs', pho);
...
logger.get('/bots/status', async (req, res) => {
  ...
  // --- NUEVA LÓGICA DE INICIO DE SERVIDOR ---
  const server = logger.listen(PORT, () => {
      global.webServerRunning = true;
      console.log(chalk.cyan(`[ ✿ ] Servidor Web iniciado en el puerto ${PORT}`));
  });

  server.on('error', (err) => {
      if (err.code === 'EADDRINUSE') {
          global.webServerRunning = true;
          console.log(chalk.yellow(`[ ! ] El puerto ${PORT} ya está en uso. El bot continuará sin abrir el servidor web de nuevo.`));
      } else {
          console.error(err);
      }
  });
